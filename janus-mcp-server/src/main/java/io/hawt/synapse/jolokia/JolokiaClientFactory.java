...
public class JolokiaClientFactory implements JolokiaServiceFactory {
    ...
    private class JolokiaClient implements JolokiaService {
        ...
        private static final String CLUSTER_CA_PATH = "/var/run/secrets/kubernetes.io/serviceaccount/ca.crt";
        ...
        public JolokiaClient(
            ...
            this.testConnection(jolokiaUrl, authToken);
        ...
        private SSLContext createPermissiveTLSContext() throws Exception {
            // --- Part 1: Create the KeyManager (presents our client certificate) ---
            ...
            // --- Part 2: Create the permissive "trust-all" TrustManager ---
            ...
            // --- Part 3: Combine them into the final SSLContext ---
            SSLContext sslContext = SSLContext.getInstance("TLS");
            sslContext.init(keyManagers, trustAllCerts, new java.security.SecureRandom());
            return sslContext;
        }

        private void testConnection(String url, String bearerToken) throws Exception {
            LOG.info("--- Running Direct SSLContext Test ---");
            // Use the exact same SSLContext we build for J4pClient
            SSLContext sslContext = createPermissiveTLSContext();

            URL testUrl = new URL(url);
            HttpsURLConnection conn = (HttpsURLConnection) testUrl.openConnection();

            // Apply our custom SSL context and hostname verifier
            conn.setSSLSocketFactory(sslContext.getSocketFactory());
            conn.setHostnameVerifier((hostname, session) -> true); // Equivalent to NoopHostnameVerifier

            // Add the bearer token
            conn.setRequestProperty("Authorization", "Bearer " + bearerToken);
                
            LOG.infof("Direct connection initiated to {}", url);
            conn.connect();

            int responseCode = conn.getResponseCode();
            LOG.infof("Direct connection successful! Response Code: {}", responseCode);
            conn.disconnect();
        }
