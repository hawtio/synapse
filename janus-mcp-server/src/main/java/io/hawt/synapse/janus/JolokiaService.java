...
public class JolokiaService {
	...
	private static final Logger LOG = Logger.getLogger(JolokiaService.class);
	...
	public URL connectionURL(Pod pod) throws Exception {
		...
		for (Container container : containers) {
			...
			try {
				...
				throw new Exception(
						String.format("Pod (%s) container (%s) is a jolokia application but failed to extract its jolokia url", pod.getFullResourceName(), container.getName()),
						e);
			}
		}
