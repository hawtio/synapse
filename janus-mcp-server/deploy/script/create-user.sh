...
if [ -z "${JANUS_USER}" ]; then
  JANUS_USER="janus-user"
fi

#
# Creating a ServiceAccount
#
${KUBE} get sa ${JANUS_USER} > /dev/null
if [ "$?" == "0" ]; then
  echo "Skipping Service Account ... (${JANUS_USER}) already exists"
else
  echo "Creating Service Account ${JANUS_USER} ..."
...
#
${KUBE} get ClusterRoleBinding ${JANUS_USER} > /dev/null
if [ "$?" == "0" ]; then
  echo "Skipping Cluster Role Binding ... (${JANUS_USER}) already exists"
else
  echo "Creating Cluster Role Binding for ${JANUS_USER} ..."
  cat <<EOF | ${KUBE} apply -f -
...
#
echo "Creating bearer token for ${JANUS_USER} ..."
${KUBE} -n ${NAMESPACE} create token ${JANUS_USER}
